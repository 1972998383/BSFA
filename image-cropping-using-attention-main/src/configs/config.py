...
_C.MODEL.TRANSFER_TYPE = "prompt"  # 迁移类型，可以是linear, end2end, prompt, adapter, side, partial-1, tinytl-bias之一
_C.MODEL.WEIGHT_PATH = ""  # 检查点文件路径
_C.MODEL.SAVE_CKPT = True  # 是否保存检查点
...
# Adapter options
# ----------------------------------------------------------------------
_C.MODEL.ADAPTER = CfgNode()  # 适配器配置节点
_C.MODEL.ADAPTER.REDUCATION_FACTOR = 8  # 缩减因子
_C.MODEL.ADAPTER.STYLE = "Pfeiffer"  # 适配器风格
...
_C.DATA.FEATURE = "sup_vitb16_imagenet21k"  # 特征，例如inat2021_supervised

_C.DATA.PERCENTAGE = 1.0  # 数据集使用百分比
_C.DATA.NUMBER_CLASSES = 15  # 类别数量
_C.DATA.MULTILABEL = False  # 是否为多标签分类
_C.DATA.CLASS_WEIGHTS_TYPE = "none"  # 类别权重类型

_C.DATA.CROPSIZE = 128  # 裁剪大小

_C.DATA.NO_TEST = False  # 是否没有测试数据
_C.DATA.BATCH_SIZE = 128  # 批处理大小

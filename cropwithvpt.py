...
def train(args, epoch, model, criterion1, criterion2, optimizer, trainloader, learning_rate, use_gpu):
    ...
    for batch_idx, (images_train, labels_train, images_test, labels_test, pids_1, pids_2) in enumerate(
        ...
        trainer = Trainer()
        s1, s2, glo1, glo2 = trainer.duenei(model, images_train, labels_train_1hot, images_test, labels_test_1hot, args)  # 使用模型进行前向传播
...
if __name__ == '__main__':
    ...
    parser.add_argument('--height', type=int, default=224, help="height of an image (default: 224)")  # 图像高度
    parser.add_argument('--width', type=int, default=224, help="width of an image (default: 224)")  # 图像宽度
    ...
    # 裁剪
    # ************************************************************
    parser.add_argument("--patch_size", default=16, help="dino patch size")
    parser.add_argument("--threshold", type=float, default=0.4, help="Threshold (set 0 to disable)")  # 注意力图阈值
    parser.add_argument("--sum_span", type=int, default=30, help="Sum span")  # 卷积核大小，用于计算得分
    parser.add_argument("--output_width", type=int, default=480, help="Output image width")  # 输出图片的宽度
    parser.add_argument("--output_height", type=int, default=360, help="Output image height")  # 输出图片的高度
